...
function canQueenCaptureKing(queen, king) {
  const rise = king.y - queen.y;
  const run = king.x - queen.x;
  const diagonal = Math.abs(rise / run) === 1;
  if (queen.x === king.x || queen.y === king.y || diagonal) return true;
  return false;
}
...
function convertToRomanNumerals(num) {
  ...
  const second = num % 10;
  if (first === 1) result = 'X';
  if (first === 2) result = 'XX';
  if (first === 3) result = 'XXX';
  switch (second) {
    case 9:
      result = `${result}IX`;
      break;
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    if (unique.has(curr)) {
      curr = unique.get(curr);
    } else {
      const temp = curr;
      curr = singleShuffle(curr);
      unique.set(temp, curr);
    }
  }
